...
class BasicThreatDetector:
    ...
    def __init__(self, p_alert=0.6, learning_rate=0.5, max_dist_to_observe_threat=600):
        self.intruders = {}
    ...
    def update_detector(self,
        ...
        with lock:
            if intruder_id not in self.intruders:
                is_threat = rospy.get_param('swarm_simulation/intruder_params/intruder_{}/is_threat'.format(intruder_id))
            ...
            if dist_to_intruder<self.max_dist_to_observe_threat:
                ...
                self.intruders[intruder_id].threat_evidence += delta_time_secs*(self.lr*(1-dist_to_intruder/self.max_dist_to_observe_threat) + noise)
                self.intruders[intruder_id].threat_evidence = max(min(self.intruders[intruder_id].threat_evidence, 1), 0)
